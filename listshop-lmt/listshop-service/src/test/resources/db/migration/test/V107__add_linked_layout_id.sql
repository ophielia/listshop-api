...
update list_layout set linked_layout_id = 5;

with base_layouts as (select *
                      from list_category
                      where layout_id = 5),
     update_mapping as (select b.name as linked_id, b.category_id, us.name, us.category_id as to_update, ll.user_id, *
                        from base_layouts b
...
set linked_category_id = um.linked_id
