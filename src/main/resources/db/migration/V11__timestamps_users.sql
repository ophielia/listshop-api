...
ALTER TABLE tag
  ADD COLUMN updated_on timestamp with time zone;
ALTER TABLE tag
  ADD COLUMN category_updated_on timestamp with time zone;
ALTER TABLE tag
  ADD COLUMN removed_on timestamp with time zone;

-- new column in tag statistics
alter table list_tag_stats
  add column added_to_dish integer;
...
-- dish
alter table dish
  add column last_added_temp timestamp;
update dish
set last_added_temp = last_added;
ALTER TABLE dish
...
alter table dish
  drop column last_added_temp;

alter table dish
  add column created_on_temp timestamp;
update dish
set created_on_temp = created_on;
ALTER TABLE dish
...
alter table dish
  drop column created_on_temp;

-- item
alter table list_item
  add column added_on_temp timestamp;
update list_item
set added_on_temp = added_on;
ALTER TABLE list_item
...
alter table list_item
  drop column added_on_temp;


alter table list_item
  add column crossed_off_temp timestamp;
update list_item
set crossed_off_temp = crossed_off;
ALTER TABLE list_item
...
alter table list_item
  drop column crossed_off_temp;


-- meal plan

alter table meal_plan
  add column created_temp timestamp;
update meal_plan
set created_temp = created;
ALTER TABLE meal_plan
...
alter table target
  add column last_updated_temp timestamp;
update target
set last_updated_temp = last_updated;
ALTER TABLE target
...
alter table target
  drop column last_updated_temp;


alter table target
  add column expires_temp timestamp;
update target
set expires_temp = expires;
ALTER TABLE target
